...
def ttt_batch_backward(
    ...
    for checkpoint_idx in range(K - 1, -1, -1):
        W1_checkpoint_offset = batch * NH * K_F_F_stride + head * K_F_F_stride + checkpoint_idx * F_F_stride + tl.arange(
            0, F)[:, None] * F + tl.arange(0, F)[None, :]
        b1_checkpoint_offset = batch * NH * K_F_stride + head * K_F_stride + checkpoint_idx * F_stride + tl.arange(0,
                                                                                                                   1)[:,
                                                                                                         None] * F + tl.arange(
            0, F)[None, :]
        ...
        for mini_batch_idx_in_group in range(0, checkpoint_group_size):
            ...
            CS_F_offset = batch * NH * NC * CS_F_stride + head * NC * CS_F_stride + mini_batch_idx * CS_F_stride + tl.arange(
                0, CS)[:, None] * F + tl.arange(0, F)[None, :]
            CS_CS_offset = batch * NH * NC * CS_CS_stride + head * NC * CS_CS_stride + mini_batch_idx * CS_CS_stride + tl.arange(
                0, CS)[:, None] * CS + tl.arange(0, CS)[None, :]
            last_CS_offset = batch * NH * NC * CS_CS_stride + head * NC * CS_CS_stride + mini_batch_idx * CS_CS_stride + (
                    CS - 1) * CS + tl.arange(0, CS)[:, None]
