...
def ttt_batch_forward(
    ...
    for i in range(NC):
        if i % checkpoint_group_size == 0:
            ...
            W1_checkpoint_offset = batch * NH * K_F_F_stride + head * K_F_F_stride + curr_k * F_F_stride + tl.arange(0,
                                                                                                                     F)[
                                                                                                           :,
                                                                                                           None] * F + tl.arange(
                0, F)[None, :]
            b1_checkpoint_offset = batch * NH * K_F_stride + head * K_F_stride + curr_k * F_stride + tl.arange(0, 1)[:,
                                                                                                     None] * F + tl.arange(
                0, F)[None, :]
        ...
        CS_F_offset = (
            batch * NH * NC * CS_F_stride + head * NC * CS_F_stride + i * CS_F_stride + tl.arange(0, CS)[:,
                                                                                        None] * F + tl.arange(0, F)[
                                                                                                    None, :]
        )
        CS_CS_offset = (
            batch * NH * NC * CS_CS_stride + head * NC * CS_CS_stride + i * CS_CS_stride + tl.arange(0, CS)[:,
                                                                                           None] * CS + tl.arange(0,
                                                                                                                  CS)[
                                                                                                        None, :]
        )
        last_CS_offset = batch * NH * NC * CS_CS_stride + head * NC * CS_CS_stride + i * CS_CS_stride + (
